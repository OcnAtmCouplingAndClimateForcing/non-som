...
wd <- getwd()
dir.output <- file.path(wd,"output","CurryTesting","Simple")
...
for(fit.species in species) {
  for(var in vars) {
    ...
    stan.fit <- NULL
    if(fit==TRUE) {
      stan.fit <- stan(file=file.path(dir.mods,"hier-Ricker-simple.stan"),
                       model_name="Hierarchical-Breakpoint-Ricker-simple",
